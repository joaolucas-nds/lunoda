...
`smoke.mjs` cobre o `entryToMD()` e o frontmatter YAML — funções puras dado um `db`. `history.mjs` monta um editor de mentira e exercita o motor de undo/redo. Ambos rodam em segundos e falham com mensagem clara. Detalhes em `tests/README.md`.
...
> Se um teste falhar, **investigue antes de mexer no código**. Já aconteceu duas vezes de o teste estar errado e o código certo: uma fixture com um único bloco que fingia um bug, e o recortador de funções que não entendia literais de regex (FIX-003).
