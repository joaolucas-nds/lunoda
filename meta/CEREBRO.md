...
- **Teto:** as Instruções não devem passar de ~6.900 caracteres. Ao propor uma mudança, diga o tamanho antes e depois.
...
## Específico deste projeto (Lunoda)

> Comportamento que só vale aqui. O resto deste arquivo é o kit genérico.

**O produto é um arquivo `.html` único.** `Lunoda_vN.html` carrega HTML+CSS+JS num só arquivo, sem build e sem dependências. Isso é decisão consciente (DEC-001), não dívida técnica — não proponha framework, bundler ou backend sem que o usuário levante o assunto.

**Antes de mexer no código, confira estas armadilhas** (detalhe no CONTEXT):
- `STORAGE_KEY = 'base_v4'` — não renomeie sem rotina de migração: o usuário perderia os dados salvos.
- `editProps` é **legado**; a fonte de verdade das propriedades é o bloco (`blockProps`), e a entrada é calculada por `calcPropertyValues()`. Não volte a usar `editProps` para decidir o que a entrada "tem".
- **Nunca deixe uma entrada com 0 blocos** — ao mover/excluir todos, recrie um bloco vazio.
- Exportações ainda usam prefixo `base_` (nome antigo do app). Ao renomear para `lunoda_`, confira também nomes vizinhos e concordância.

**Os guias de versão são memória, não lixo.** Cada `lunoda_vN_guide.md` registra o raciocínio de uma versão para a outra e é o que permite reconstruir contexto quando o mount some. Não descarte nem sobrescreva guias antigos.

**Ao propor mudança no `.html`:** diga o que testar (caso feliz, borda, regressão) e quais telas valem print para o README — a ferramenta é visual e não tem suíte de testes.

**Entrada composta é a norma aqui.** O usuário costuma juntar 4–6 pedidos numa mensagem só, misturando bug, funcionalidade nova, dúvida de viabilidade e planejamento. Enumere as partes, execute o que não bloqueia e pare só no que de fato trava — não deixe um pedido soterrar os outros.

---

*Gerado pelo Kit de Contexto Universal v1.74.0 — nicho Desenvolvimento. Edite à vontade: este arquivo é seu.*
*Customizado para o projeto Lunoda em 2026-07-19 (ver seção «Específico deste projeto»).*